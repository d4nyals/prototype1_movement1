...
class Player:  # player class
    def __init__(self, pos):
        ...
        self.healthBar = 100   # used for drawing the health bar
        self.score = 0         # inner score (personal note)
        self.scoreBar = 0      # used for drawing the score
    ...
    def drawHealthBar(self, screen):  # draws health bar at bottom right
        bar_width = 200
        bar_height = 20
        x = width - bar_width - 20  # 20px margin from right
        y = height - bar_height - 20 # 20px margin from bottom

        health_ratio = self.health / self.max_health
        self.healthBar = bar_width * health_ratio

        # Draw background bar (grey)
        pygame.draw.rect(screen, (100, 100, 100), (x, y, bar_width, bar_height))
        # Draw foreground health bar (red)
        pygame.draw.rect(screen, (255, 0, 0), (x, y, self.healthBar, bar_height))

        # Draw the text "Health: current/max"
...
class Bullet:  # bullet class
    ...
    def moveBullet(self, walls): # bullet movement
        if self.direction == "up":  # bullet goes up
            ...
            self.rect.x += bulletSpeed
        for wall in walls: # stops a bullet if it hits a wall
            if self.rect.colliderect(wall.rect):
                return True  # boolean to remove bullet
        return False
...
class House:  # house structure class, primarily used for dimensions of the house
    def __init__(self, x, y, width, height, door_width=60, door_side=None, vertical_door=None):  # creates the walls for house and adds the door gaps
        ...
        self.door_side = door_side # which side contains a horizontal door gap
        self.vertical_door = vertical_door # which side contains a vertical door gap
...
class Game:  # game class
    ...
    def setupWindow(self):  # method which is used to create the game window
        self.screen = pygame.display.set_mode((width, height))  # creates the main game window (surface object)
    ...
    def menuScreen(self):  # displays start menu before the main game begins
        playButton = pygame.Rect(width // 2 - 100, height // 2 - 25, 200, 50) # creates a rect for a play button
        ...
        while runningMenu: # main menu loop  (runs until runningMenu is False)
            ...
            for event in pygame.event.get(): # checks every event (keyboard, mouse)
                if event.type == pygame.QUIT:
                    ...
                    if playButton.collidepoint(mouse_pos): # checks if mouse click happened inside button
    ...
    def structures(self): # creates structures, such as houses for the player to use as cover
        ...
        top_left_house = House(200, 120, 300, 150, door_side='bottom', vertical_door='left')  # first house with 2 doors
        self.walls.add(top_left_house.walls) # adds all wall sprites from first house into main wall group
        bottom_right_house = House(900, 400, 180, 180, door_side='top', vertical_door='right')  # second house with 2 doors
    ...
    def drawScoreBar(self):  # draw score bar at top-left
        scoreBar_rect = pygame.Rect(20, 20, 150, 40)  # rectangle for score background
        self.scoreBar = self.player.score  # update variable for drawing
        pygame.draw.rect(self.screen, (100, 100, 100), scoreBar_rect)
        font = pygame.font.Font(None, 30) # Draws score text
        text = font.render(f"Score: {self.scoreBar}", True, (255, 255, 255))
        self.screen.blit(text, (scoreBar_rect.x + 10, scoreBar_rect.y + 5))

    def draw(self):  # draw background and objects
        self.screen.fill(background_colour)
        for wall in self.walls: # draws wall
            self.screen.blit(wall.image, wall.rect)
        for bullet in self.bullets: # draws bullet
            bullet.draw(self.screen)
        self.player.draw(self.screen)
        self.player.drawHealthBar(self.screen) # draws health bar
        self.drawScoreBar() # draws score bar
